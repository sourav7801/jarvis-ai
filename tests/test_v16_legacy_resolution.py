...
class LegacyResolutionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.paths = patch.multiple(
            guard,
            TERMINAL_DB=self.desk.db_path,
            LEGACY_ACCOUNT=self.path,
            LEGACY_SPREADS=self.root / "absent.sqlite3",
        )
        self.paths.start()
        ...
        self.record = dict(
            ...
            stop_loss=90.0,
            take_profit=130.0,
            opened_at="2026-08-17T23:34:17",
            current_price=105.0,
            realized_pnl=0.0,
            private_note="not-for-browser",
            metadata={"secret": "not-for-browser"},
        )
        self.source = dict(
            version=2,
            positions=[self.record],
            account=dict(cash=800.0, realized_pnl=0.0, total_fees=0.0, starting_capital=1000.0),
            trades=[],
        )
        ...
        self.spec_patch = patch.object(
            resolution,
            "instrument_status",
            side_effect=lambda symbol: dict(status="VALID", spec=dict(self.spec), reason=None),
        )
    ...
    def test_diagnostics_sanitized_and_take_profit_preserved(self):
        ...
        result = resolution.legacy_resolution_details(self.runtime)
        row = result["positions"][0]
        self.assertEqual(row["entry_price"], 100.0)
        self.assertEqual(row["target"], 130.0)
    ...
    def test_migration_preserves_economics_and_is_idempotent(self):
        ...
        with patch.object(self.desk, "open_position", side_effect=AssertionError("Migration is not an entry")):
            first = self.run_action(body)
            second = self.run_action(body)
    ...
    def test_ack_failure_stays_blocked_and_retry_never_duplicates(self):
        ...
        with patch.object(resolution, "_write_source", side_effect=OSError("file locked")):
            result = self.run_action(body)
        self.assertFalse(result["success"])
        self.assertEqual(result["reason"], "SOURCE_ACK_PENDING")
        self.assertEqual(self.path.read_bytes(), before)
        self.assertEqual(len(self.rows()), 1)
    ...
    def test_close_uses_verified_exit_and_updates_source_account(self):
        result = self.run_action(self.body("CLOSE"))
        self.assertTrue(result["success"], result)
        self.assertEqual(self.rows(), [])
        data = json.loads(self.path.read_text())
        self.assertEqual(data["positions"], [])
        audit = data["resolved_positions"][0]["resolution"]
        self.assertEqual(audit["close_price"], 110.0)
        self.assertEqual(audit["pnl"], 20.0)
        self.assertEqual(data["account"]["cash"], 1020.0)
        self.assertEqual(data["account"]["realized_pnl"], 20.0)
        self.assertEqual(guard.legacy_exposure(self.desk), [])

    def test_stale_identity_and_closed_market_cannot_fill(self):
        for change, reason in [
            ({"exchange_timestamp": (datetime.now(timezone.utc) - timedelta(minutes=4)).isoformat()}, "STALE_MARK"),
            ({"symbol": "ETH"}, "QUOTE_INSTRUMENT_MISMATCH"),
        ]:
            with self.subTest(reason=reason):
                previous = dict(self.quote)
        ...
        with patch.object(
            resolution,
            "verified_exit",
            return_value={"success": False, "reason": "MARKET_SESSION_CLOSED"},
        ):
    ...
    def test_expired_unresolved_and_unit_mismatch_fail_closed(self):
        for state in ("EXPIRED", "UNRESOLVED", "DELISTED"):
            with patch.object(
                resolution,
                "instrument_status",
                return_value={"status": state, "spec": {}, "reason": state},
            ):
                details = resolution.legacy_resolution_details(self.runtime)["positions"][0]
                self.assertFalse(details["can_migrate"])
                self.assertFalse(details["can_close"])
                self.assertFalse(self.run_action(self.body("CLOSE"))["success"])
        self.spec.update(instrument_type="OPTION", contract_multiplier=100.0)
    ...
    def test_equivalent_canonical_exposure_is_not_imported_again(self):
        ...
        self.assertFalse(result["success"])
        self.assertEqual(result["reason"], "CANONICAL_EXPOSURE_ALREADY_EXISTS")
        self.assertEqual(len(self.rows()), 1)
    ...
    def test_closed_session_is_rechecked_by_real_quote_certificate(self):
        with patch(
            "workstation.terminal_data.VENUE_SESSIONS.evaluate",
            return_value=SimpleNamespace(as_dict=lambda: dict(session_open=False)),
        ):
            before = self.path.read_bytes()
            self.assertEqual(self.run_action(self.body("CLOSE"))["reason"], "MARKET_SESSION_CLOSED")
            self.assertEqual(self.path.read_bytes(), before)
    ...
    def test_acknowledgement_crash_after_json_replace_is_retryable(self):
        ...
        write = resolution._write_source
    ...
    def test_legacy_writer_cannot_resurrect_migrated_position(self):
        ...
        old.state_file = self.path
        old.broker = PaperBroker()
        old.broker.positions = {
            "BTC": Position(
                symbol="BTC",
                side="LONG",
                quantity=2,
    ...
    def test_http_requires_local_token_and_never_starts_session(self):
        ...
        class Base(BaseHTTPRequestHandler):
            def _local(self):
                return self.headers.get("Origin", "").startswith("http://127.0.0.1")
        ...
        try:
            ...
            for headers in (
                ...
                {"Origin": "https://example.com", "X-Jarvis-Token": self.runtime.token},
            ):
                with self.assertRaises(urllib.error.HTTPError) as caught:
                    urllib.request.urlopen(urllib.request.Request(url, raw, headers), timeout=5)
                self.assertEqual(caught.exception.code, 403)
                caught.exception.close()
            self.assertEqual(self.rows(), [])
            headers = {
                "Origin": "http://127.0.0.1",
                "X-Jarvis-Token": self.runtime.token,
            }
    ...
    def test_canonical_option_buy_after_resolution_still_requires_start(self):
        ...
        self.quote.update(symbol=symbol, provider="FYERS_DATA", bid=99.5, ask=100.0, ltp=100.0)
        spec = {
            **self.spec,
            "symbol": symbol,
        ...
        body = dict(
            ...
            target=120,
            lots=1,
            client_order_id="after-explicit-resolution",
        )
        ...
        def certificate(symbol, payload, **kwargs):
            return real_certificate(symbol, payload, now=market_now, **kwargs)

        self.runtime.mark_loader = lambda symbol: certificate(symbol, self.quote)
        with patch.object(
            terminal_data,
            "quote_certificate",
            side_effect=certificate,
        ), patch.object(
            terminal_data.VENUE_SESSIONS,
            "evaluate",
            return_value=SimpleNamespace(as_dict=lambda: dict(session_open=True)),
        ), patch.object(
            v16_option_paper,
    ...
    def test_session_ui_uses_entry_session_not_overall_problem(self):
        js = (
            ...
            / "workstation/quant_terminal_v2_static/v16_option_execution.js"
