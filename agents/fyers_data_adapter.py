...
V16 retains the proven V15.1 cross-process provider governor because JARVIS
analysis runs FYERS history calls from the isolated .venv-fyers process. The
governor is strictly data-side: it serializes REST calls, caches recent
successful history snapshots, and fails closed during a verified FYERS 429
cooldown. It never creates candles and never exposes broker order methods.
"""
...
def provider_governor_status() -> dict[str, Any]:
    ...
    return {
        "success": True,
        "service": "JARVIS_FYERS_DATA_GOVERNOR_V16",
