"""JARVIS V11 runtime wrapper.

Preserves V6.2 Quant ownership protection and V8 Master identity checks, then
adds V11 Completion Center identity protection so an older trusted 8799 process
cannot be silently adopted as the current cognitive-execution surface. Unknown
processes are never terminated. No broker or order API is imported here.
"""
...
def completion_v11_surface_status() -> dict[str, Any]:
    status, raw = _http(COMPLETION_BASE + "/api/v11/status")
    ...
    current = bool(
        ...
        and payload.get("version") == "11.0"
        and payload.get("service") == "JARVIS_COGNITIVE_EXECUTION_CONVERGENCE"
...
def reclaim_obsolete_completion_listener() -> dict[str, Any]:
    ...
    surface = completion_v11_surface_status()
    if surface.get("current"):
        return {"action": "CURRENT_V11_COMPLETION_PRESENT", "surface": surface, "stopped": []}

    pids = _listener_pids(COMPLETION_PORT)
    if not pids:
        raise RuntimeError(
            "Port 8799 is occupied by a non-V11 surface, but its owning PID could not be resolved. "
    ...
    for pid in pids:
        ...
        print(f"V11 PREFLIGHT > stopping obsolete trusted Completion listener PID {pid}")
        _kill_tree(pid)
        stopped.append(pid)
...
def v11_services(root: Path = ROOT) -> tuple[ManagedService, ...]:
    ...
    for service in v8_services(root):
        ...
        services.append(
            ManagedService(
                name=service.name,
                argv=service.argv,
                health_url=COMPLETION_BASE + "/api/v11/status",
                expected_service="JARVIS_COGNITIVE_EXECUTION_CONVERGENCE",
                port=service.port,
                health_markers=(),
...
def main() -> int:
    print("JARVIS V11 runtime preflight...")
    ...
    print("Starting V11 supervised services: Master, Quant, Nautilus, Completion Center.")
