    ...
    for start_index in range(len(tokens)):
        for width in (1, 2, 3):
            ...
            for alias, symbol in compact_aliases:
                if not alias or abs(len(window) - len(alias)) > 2:
                    continue
                if window == alias:
                    best_symbol = symbol
                    best_ratio = 1.0
...
def is_paper_trade_action_request(text: str) -> bool:
    from workstation.paper_trading_desk import paper_command_kind

    # Paper desk / portfolio / autonomy commands own their domain and must not
    # be hijacked by the generic phrase "paper trading".
    if paper_command_kind(text) is not None:
        return False
    value = normalize_text(text)
...
def execute_paper_trade_request(text: str) -> dict[str, Any]:
    ...
    if not symbols:
        return {
            ...
            "speech": (
                ...
                "Say, for example, 'take trade in Bitcoin' or 'paper trade Nifty'. No trade was placed."
            ),
            "paper_only": True,
        ...
        _publish_event(
            {
                ...
                "symbol": symbol,
                "provider": "JARVIS_ADAPTIVE_QUANT_V6",
    ...
    if entry is None:
        ...
        return {
            ...
            "autonomy": autonomy,
            "speech": (
                f"{symbol} produced a candidate, but live-entry validation rejected immediate paper entry: {live_rejection}. "
    ...
    if not result.get("success"):
        ...
        return {
            ...
            "autonomy": autonomy,
            "speech": (
                f"{symbol} had a qualified setup, but the portfolio risk gate rejected entry: {result.get('reason')}. "
    ...
    return {
        ...
        "speech": (
            ...
            f"adaptive Quant score {float(best.get('score') or 0):.1f}. Live broker execution remains locked."
        ),
        "paper_only": True,
        "live_execution": False,
    }
