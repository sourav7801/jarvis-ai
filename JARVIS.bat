...
title JARVIS OS V8 MASTER + V13 ADAPTIVE INTELLIGENCE OPERATING SYSTEM

set "JARVIS_PY=C:\Jarvis\.venv\Scripts\python.exe"
if exist "%JARVIS_PY%" "%JARVIS_PY%" -c "import omni, numpy, pandas, workstation.jarvis_os_v3, workstation.jarvis_os_v8, workstation.completion_console_v13" >nul 2>&1
...
REM JARVIS_RUNTIME_SUPERVISOR_V13
...
REM Preserves V6.2 stale-Quant ownership, V8 protected Master identity, V11 execution mesh and V12 adaptive EV.
REM V13 refuses to silently adopt stale Quant/Completion surfaces without contextual authority.
REM Master remains the protected V8 Unified Intelligence HTTP/UI runtime on port 8797.
REM Quant uses contextual expected value + closed-paper outcome memory + completed-bar correlation risk.
REM No live broker-order surface is added by this launcher.
"%JARVIS_PY%" -m scripts.jarvis_runtime_supervisor_v13

if errorlevel 1 (
    echo.
    echo JARVIS V13 exited with an error.
    pause
)
